defmodule Blog.TursoHttpClient do
  ...
  defp get_config do
    case {System.get_env("LIBSQL_URI"), System.get_env("LIBSQL_TOKEN")} do
      {nil, _} ->
        {:error, "LIBSQL_URI not configured"}

      {_, nil} ->
        {:error, "LIBSQL_TOKEN not configured"}
  ...
  defp decode_value(%{"type" => "blob", "base64" => base64}) do
    case Base.decode64(base64) do
      {:ok, binary} -> binary
      # Return the base64 string if decoding fails
      :error -> base64
    end
  end
  ...
  defp decode_value(%{"type" => "float", "value" => value}) when is_binary(value),
