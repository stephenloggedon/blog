defmodule BlogWeb.HomeLiveTest do
  ...
  describe "HomeLive search functionality" do
    ...
    test "multiple tag selection with OR logic", %{conn: conn} do
      ...
      assert rendered =~ "Phoenix LiveView Guide"  # has both elixir and web
      assert rendered =~ "Elixir Patterns"         # has elixir
      assert rendered =~ "Web Development"         # has web
    end
    ...
    test "remove_tag event removes specific tag from selection", %{conn: conn} do
      ...
      rendered = render(view)
      assert rendered =~ "Phoenix LiveView Guide"  # has web
      assert rendered =~ "Web Development"         # has web
      refute rendered =~ "Elixir Patterns"         # only has elixir
    end
    ...
    test "search with non-tag text performs content search", %{conn: conn} do
      ...
      rendered = render(view)
      assert rendered =~ "Phoenix LiveView Guide"  # has "guide" in title
      refute rendered =~ "Elixir Patterns"         # no "guide"
      refute rendered =~ "Web Development"         # no "guide"
    end
    ...
    test "clear_filters event removes all tags and search", %{conn: conn} do
      ...
      # Click clear button (just take the first one found)
      elements = view |> element("button[phx-click='clear_filters']") |> render_click()
    ...
    test "URL parameters are properly parsed on mount", %{conn: conn} do
      ...
      assert rendered =~ "Phoenix LiveView Guide"  # has elixir+web and "guide"
      refute rendered =~ "Elixir Patterns"         # has elixir but no "guide"
      refute rendered =~ "Web Development"         # has web but no "guide"
    end
